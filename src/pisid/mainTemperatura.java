...
public class mainTemperatura extends MongotoMongo{
		...
		while(true) {
           ...
           String docStr1 = cursor.toString();
			...
			if(Double.parseDouble(leitura1)>  mn && Double.parseDouble(leitura1)< mx ) {
				...
				if(total_zonas_temperatura.contains(leitura2)) {
           ...
           Document cursor_individual = temperatura.find(comp).sort(new BasicDBObject("_id", -1)).first();
