...
public class mainHumidade extends MongotoMongo{
		...
		while(true) {
           ...
           String docStr1 = cursor.toString();
			...
			if(Double.parseDouble(leitura1)>  mn && Double.parseDouble(leitura1)< mx ) {
				...
				if(total_zonas_humidade.contains(leitura2)) {
				zonas.add(leitura2);
           ...
           Document cursor_individual = humidade.find(comp).sort(new BasicDBObject("Data", -1)).first();
